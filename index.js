...
class Router {
  constructor() {
    ...
    _classPrivateMethodInitSpec(this, _escapeSpecialChars);
    /* Replaces syntax {wildcard} with .+ regex wildcard
        /route/{wildcard} -> /route/.+
    */
    _classPrivateMethodInitSpec(this, _escapeWildCard);
    _classPrivateMethodInitSpec(this, _getPathRegex);
    _classPrivateMethodInitSpec(this, _onHashChange);
...
class XHR {
  constructor() {
    ...
    _classPrivateFieldInitSpec(this, _setDefaultArgs, {
      writable: true,
    ...
    _classPrivateFieldInitSpec(this, _setEvents, {
      writable: true,
    ...
    _classPrivateFieldInitSpec(this, _setCallbackChains, {
      writable: true,
      value: function value(xhr) {
    ...
    _classPrivateFieldInitSpec(this, _setHeaders, {
      writable: true,
      value: function value(xhr) {
        var args = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
...
class DataBinding {
  constructor() {
    ...
    _classPrivateMethodInitSpec(this, _addEventListeners$1);
    ...
    */
    _classPrivateMethodInitSpec(this, _setNested);
    /* Sets each property to itself to activate the setters. 
        For a nested value { a : { b : { c : 1 } } }
        It sets proxy['a.b.c'] = proxy['a']['b']['c'];
    */
    _classPrivateMethodInitSpec(this, _activateSetters);
    _classPrivateMethodInitSpec(this, _getProxyHandler);
  }
  bind(container, data) {
...
Lite.router.addRoutes([{
  route: '',
  value: () => {
    import('./lookups-c8a627c2.js').then(load);
  }
}, {
  route: 'lookups',
  value: () => {
  ...
  value: () => {
    import('./battle-manager-6365716c.js').then(load);
  }
}, {
  route: 'encounter-builder',
  value: () => {
    import('./encounter-builder-7dc8d94d.js').then(load);
  }
}, {
  route: 'character-sheets',
  value: () => {
    import('./character-sheets-034e83ee.js').then(load);
  }
}, {
  route: 'character-sheet/{name}',
  value: () => {
    import('./character-sheet-fcd9db00.js').then(load);
  }
}]);
  ...
  */
  new DiceOperation({
    ...
    resolve: function resolve(repetitions, facets) {
      ...
      var results = Array(+repetitions).fill('d' + facets).map(x => +operation.parent.solve(x));
      // high-to-low sorting
...
var MathModule = function MathModule() {
  ...
  /* Needs to happen simultaneously, so a single function */, new DiceOperation({
    name: 'MultiplyDivide',
    search: new RegExp(rgxNumber + '[*\\/]' + rgxNumber),
    parse: function parse(expression) {
    ...
    parse: function parse(expression) {
      var firstOperand = new RegExp('^' + rgxNumber).exec(expression)[0];
      var secondOperand = new RegExp(rgxNumber + '$').exec(expression)[0];
      var operator = expression.substr(firstOperand.length, 1);
...
var paging = function paging() {
  ...
  grid.paging = {
    ...
    _setRowVisibility: function _setRowVisibility(pageNumber) {
      ...
      var end = options.currentPage * options.rows;
...
class DiceHeader {
  constructor(args) {
    _classPrivateMethodInitSpec(this, _getLogItem);
    _classPrivateMethodInitSpec(this, _addEventListeners);
    _classPrivateMethodInitSpec(this, _setElements);
...
function _getLogItem2(equation, solution) {
  var logItem = this.dice.log.slice(-1)[0];
...
export { Gridify as G, Lite as L, _classPrivateMethodInitSpec as _, _classPrivateFieldInitSpec as a, _classPrivateMethodGet as b, _classPrivateFieldGet as c };
